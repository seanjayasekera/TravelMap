...
# Optional URL fetch (not required)
try:
    import requests
    REQUESTS_OK = True
except Exception:
...
st.markdown("""
<style>
/* Make every base layer transparent so background shows through */
html, body, .stApp,
...
panel_mode = st.sidebar.selectbox(
    "Panel style",
    ["Showcase (more transparent)", "Readable (more opaque)"],
    index=0
)
...
if panel_mode.startswith("Showcase"):
    panel_rgba = "rgba(255,255,255,0.55)"  # more transparent
    sidebar_rgba = "rgba(255,255,255,0.55)"
else:
    panel_rgba = "rgba(255,255,255,0.85)"  # more opaque for readability
    sidebar_rgba = "rgba(255,255,255,0.85)"

# Minimal “glass” styling for panels
st.markdown(f"""
<style>
/* Center content a bit so edges reveal the background */
.block-container {{
